...
</nav>`;$("#nav").html(l),applyTheme(localStorage.getItem("gdi-theme")||"dark")}function render(i){i.indexOf("?")>=0&&(i=i.substr(0,i.indexOf("?"))),title(i),nav(i);const e=/\/\d+:$/g;if(i.includes("/fallback")){window.scroll_status={event_bound:!1,loading_lock:!1};const t=getQueryVariable("a"),n=decodeURIComponent(getQueryVariable("id")||"");return t?fallback(n,!0):list(null,n,!0)}else window.MODEL.is_search_page?(window.scroll_status={event_bound:!1,loading_lock:!1},render_search_result_list()):i.match(e)||i.slice(-1)=="/"?(window.scroll_status={event_bound:!1,loading_lock:!1},list(i)):file(i)}function requestListPath(i,e,t,n,a=3,c=!1){const l={id:e.id||"",type:"folder",password:e.password||"",page_token:e.page_token||"",page_index:e.page_index||0},d=`gdi_cache_${i}_${l.id}_${l.page_token}_${l.page_index}_${l.password}`;try{const r=sessionStorage.getItem(d);if(r){const p=JSON.parse(r);if(p&&p.data){setTimeout(()=>{t(p,i,l),$("#update").hide()},0);return}}}catch{}const o=a!=null&&a>=0?a:3;$("#update").show(),$("#update").html('<div class="gdi-alert gdi-alert-info">Connecting\u2026</div>'),c&&(i="/0:fallback");async function s(r){try{const p=await fetch(c?"/0:fallback":i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(l)});if(!p.ok)throw new Error("Request failed with status "+p.status);const g=await p.json();if(g&&g.error&&g.error.code===401)$("#update").hide(),askPassword(i);else if(g&&g.data===null)document.getElementById("spinner")?.remove(),$("#list").html(`<div class="gdi-empty"><i class="bi bi-exclamation-circle"></i><p>Server didn't send any data.</p></div>`),$("#update").hide();else if(g&&g.data){try{sessionStorage.setItem(d,JSON.stringify(g))}catch{}t(g,i,l),$("#update").hide()}}catch(p){r>0?($("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying\u2026 (${r} left)</div>`),await sleep(2e3),await s(r-1)):($("#update").html('<div class="gdi-alert gdi-alert-error">Unable to connect. Please try again.</div>'),$("#list").html(`<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>${escHtml(String(p))}</p></div>`),$("#update").hide())}}s(o)}function requestSearch(i,e,t=3){const n={q:i.q||null,page_token:i.page_token||null,page_index:i.page_index||0};async function a(c){try{const l=await fetch(`/${window.current_drive_order}:search`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!l.ok)throw new Error("Request failed with status "+l.status);const d=await l.json();d&&d.data===null?($("#spinner").remove(),$("#list").html('<div class="gdi-empty"><i class="bi bi-search"></i><p>No results found.</p></div>'),$("#update").remove()):d&&d.data&&(e&&e(d,n),$("#update").remove())}catch{c>0?($("#update").html(`<div class="gdi-alert gdi-alert-info">Retrying\u2026 (${c} left)</div>`),await sleep(2e3),await a(c-1)):($("#update").html(`<div class="gdi-alert gdi-alert-error">Unable to connect after ${t} attempts.</div>`),$("#list").html('<div class="gdi-empty"><i class="bi bi-wifi-off"></i><p>Connection failed.</p></div>'),$("#spinner").remove())}}$("#update").html('<div class="gdi-alert gdi-alert-info">Searching\u2026</div>'),a(t)}function list(i,e="",t=!1){const n=window.location.pathname,a=trimChar(n,"/").split("/");let c="/",l='<li><a href="/">Home</a></li><li class="gdi-bc-sep">/</li>';if(a.length>1)for(const p in a){const g=a[p];let f;try{f=decodeURIComponent(g)}catch{f=g}f=f.replace(/\//g,"%2F");const u=f.match(/^(\d+):$/),h=u&&window.drive_names&&window.drive_names[+u[1]]||f,m=h.length>18?h.slice(0,14)+"\u2026":h;if(c+=g+"/",!m)break;Number(p)===a.length-1?l+=`<li class="gdi-bc-cur" title="${escHtml(h)}">${escHtml(m)}</li>`:l+=`<li><a href="${c}" title="${escHtml(h)}">${escHtml(m)}</a></li><li class="gdi-bc-sep">/</li>`}const d=`
<div class="gdi-wrap">
  <div id="update"></div>
...
</a>`;else{const g=Number(s.size);l+=g,s.size=formatFileSize(s.size),d=!0;const f=s.fileExtension,u=s.link?UI.second_domain_for_dl?UI.downloaddomain+s.link:window.location.origin+s.link:null,h=encodeURIComponent(s.name);let m=i+h+"?a=view";const w=!window.UI||window.UI.user_role==="admin"||window.UI.can_stream||window.UI.can_download,y=!window.UI||window.UI.user_role==="admin"||window.UI.can_download,_=i+h;n&&s.name=="README.md"&&UI.render_readme_md&&get_file(_,s,function(v){markdown("#readme_md",v),$("img").addClass("img-fluid")}),s.name=="HEAD.md"&&UI.render_head_md&&get_file(_,s,function(v){markdown("#head_md",v),$("img").addClass("img-fluid")});const b=GDOC_TYPES[s.mimeType],x=b?b.icon:getFileIcon(f),k=b&&UI.display_download?b.formats.map(v=>`<a class="gdi-act-btn" href="${u}&fmt=${v.ext}" title="Export as ${v.label}" download><span style="font-size:10px;font-weight:600;">${v.label}</span></a>`).join(""):"";c+=`<div class="gdi-row countitems size_items" data-name="${escHtml(s.name.toLowerCase())}" data-bytes="${g}" data-date="${s.modifiedTime||""}">
  ${w&&UI.allow_selecting_files?`<input class="gdi-row-check" type="checkbox" value="${u}">`:""}
  <span class="gdi-row-icon">${x}</span>
  ${w?`<a class="gdi-row-name" href="${m}" title="${escHtml(s.name)}" data-size="${UI.display_size?s.size:""}">${escHtml(s.name)}</a>`:`<span class="gdi-row-name gdi-row-name--locked" title="${escHtml(s.name)}" data-size="${UI.display_size?s.size:""}">${escHtml(s.name)}</span>`}
...
`);navigator.clipboard.writeText(c).catch(()=>{const l=document.createElement("textarea");l.value=c,document.body.appendChild(l),l.select(),document.execCommand("copy"),document.body.removeChild(l)}),showToast(`${a.length} link${a.length>1?"s":""} copied`)})}function append_search_result_to_list(i){try{const e=$("#list"),t=e.data("nextPageToken")===null;let n="",a=0,c=!1;for(const l in i){const d=i[l];d.size==null&&(d.size=""),d.modifiedTime=utc2delhi(d.modifiedTime);const o=typeof d.rootIdx=="number"?d.rootIdx:-1;if(d.mimeType=="application/vnd.google-apps.folder")n+=`<a style="cursor:pointer;" onclick="onSearchResultItemClick('${escHtml(d.id)}', false, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel"
  ...
  <span class="gdi-row-acts"></span>
</a>`;else{c=!0;const s=Number(d.size);a+=s,d.size=formatFileSize(d.size);const r=d.fileExtension,p=d.link?UI.second_domain_for_dl?UI.downloaddomain+d.link:window.location.origin+d.link:null,g=!window.UI||window.UI.user_role==="admin"||window.UI.can_stream||window.UI.can_download,f=!window.UI||window.UI.user_role==="admin"||window.UI.can_download;n+=`<div class="gdi-row countitems size_items" data-name="${escHtml(d.name.toLowerCase())}" data-bytes="${s}" data-date="${d.modifiedTime||""}">
  ${g&&UI.allow_selecting_files?`<input class="gdi-row-check" type="checkbox" value="${p}">`:""}
  <span class="gdi-row-icon">${getFileIcon(r)}</span>
  <span class="gdi-row-name" ${g?`onclick="onSearchResultItemClick('${escHtml(d.id)}', true, ${o})" data-bs-toggle="modal" data-bs-target="#SearchModel" style="cursor:pointer;"`:""} data-size="${UI.display_size?d.size:""}">${escHtml(d.name)}</span>
  <span class="gdi-row-size">${UI.display_size?d.size:""}</span>
  <span class="gdi-row-date">${UI.display_time?d.modifiedTime:""}</span>
  <span class="gdi-row-acts">
    ${UI.display_download&&f&&p?`<a class="gdi-act-btn" href="${p}" title="Download"><i class="bi bi-download"></i></a>`:""}
  </span>
  ...
  <div class="gdi-viewer">
    ...
    <div class="gdi-code-outer" style="display:none;">
      <pre><code id="editor"></code></pre>
            ...
            </video>`,p="https://cdn.plyr.io/"+player_config.plyr_io_version+"/plyr.polyfilled.js",g="https://cdn.plyr.io/"+player_config.plyr_io_version+"/plyr.css"):player_config.player==="videojs"?(r=`<video id="vplayer" poster="${n}" class="video-js vjs-default-skin vjs-big-play-centered" controls preload="auto" width="100%" height="100%" data-setup='{"fluid":true}'>
              <source src="${a}" type="${s?"application/x-mpegURL":"video/mp4"}">
              <source src="${a}" type="video/webm">
              ${o.map(h=>`<track kind="subtitles" src="${escHtml(h.url)}" label="${escHtml(h.label)}" default>`).join("")}
            </video>`,p="https://vjs.zencdn.net/"+player_config.videojs_version+"/video.js",g="https://vjs.zencdn.net/"+player_config.videojs_version+"/video-js.css"):player_config.player==="dplayer"?(r='<div id="player-container"></div>',p="https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.js",g="https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.css"):player_config.player==="jwplayer"&&(r='<div id="player"></div>',p="https://content.jwplatform.com/libraries/IDzF9Zmk.js"));const f=`<div class="gdi-player-wrap">${r}</div>`,u=UI.disable_video_download?"":renderDownloadButtons(a,e,{showMedia:!0});if($("#content").html(_viewerCard('<i class="bi bi-camera-video-fill gdi-icon-video"></i>',i,t,f,u)),g){const h=document.createElement("link");h.rel="stylesheet",h.href=g,document.head.appendChild(h)}if(p){const h=document.createElement("script");h.src=p,h.onload=function(){if(player_config.player==="plyr")new Plyr("#player",{keyboard:{focused:!0,global:!0}});else if(player_config.player==="videojs"){const m=videojs("vplayer",{playbackRates:[.5,.75,1,1.25,1.5,2],controlBar:{pictureInPictureToggle:!0}});m.ready(function(){this.el().addEventListener("keydown",function(w){w.target.tagName!=="INPUT"&&(w.key===" "?(w.preventDefault(),m.paused()?m.play():m.pause()):w.key==="f"?m.isFullscreen()?m.exitFullscreen():m.requestFullscreen():w.key==="m"?m.muted(!m.muted()):w.key==="ArrowRight"?m.currentTime(m.currentTime()+10):w.key==="ArrowLeft"?m.currentTime(Math.max(0,m.currentTime()-10)):w.key==="ArrowUp"?m.volume(Math.min(1,m.volume()+.1)):w.key==="ArrowDown"&&m.volume(Math.max(0,m.volume()-.1)))})})}else player_config.player==="dplayer"?new DPlayer({container:document.getElementById("player-container"),screenshot:!0,video:{url:a,pic:n,type:s?"hls":"auto"},subtitle:o.length?{url:o[0].url,type:"webvtt"}:void 0}):player_config.player==="jwplayer"&&jwplayer("player").setup({file:a,type:c,autostart:!1,image:n,width:"100%",aspectratio:"16:9",title:i,description:"Powered by Google Drive Index",tracks:o.map(m=>({file:m.url,kind:"captions",label:m.label,default:!0})),captions:{color:"#f3f378",fontSize:14,backgroundOpacity:50,edgeStyle:"raised"}})},document.head.appendChild(h)}}function file_audio(i,e,t,n,a,c,l){l=l||[{name:i,url:n,cover:UI.audioposter}];const d=UI.disable_player?"":'<div id="aplayer-container" style="max-width:680px;margin:0 auto;"></div>',o=UI.disable_audio_download?"":renderDownloadButtons(n,e,{showMedia:!0});if($("#content").html(_viewerCard('<i class="bi bi-music-note-beamed gdi-icon-audio"></i>',i,t,d,o)),UI.disable_player)return;const s=document.createElement("link");s.rel="stylesheet",s.href="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.css",document.head.appendChild(s);const r=document.createElement("script");if(r.src="https://cdn.jsdelivr.net/npm/aplayer@1.10.1/dist/APlayer.min.js",r.onload=function(){window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,autoplay:!1,theme:"#4d9fec",loop:"all",order:"list",preload:"auto",volume:.7,listFolded:!1,audio:l})},document.head.appendChild(r),l.length<=1){const p=window.location.pathname.split("/").slice(0,-1).join("/")+"/";requestListPath(p,{},function(g){if(!g||!g.data||!g.data.files)return;const f=g.data.files.filter(u=>FILE_TYPES.audio.includes(u.fileExtension));if(f.length>1){const u=f.map(m=>({name:m.name,url:UI.second_domain_for_dl?UI.downloaddomain+m.link:window.location.origin+m.link,cover:UI.audioposter}));window._gdiAPlayer&&window._gdiAPlayer.destroy(),window._gdiAPlayer=new APlayer({container:document.getElementById("aplayer-container"),mini:!1,loop:"all",order:"list",preload:"auto",volume:.7,audio:u});const h=u.findIndex(m=>m.url===n);h>0&&window._gdiAPlayer.list.switch(h)}},null)}}function file_pdf(i,e,t,n,a,c){const l=`<div class="gdi-wrap">
  <div class="gdi-viewer">
