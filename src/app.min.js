...
</a>`;else{const g=Number(s.size);l+=g,s.size=formatFileSize(s.size),d=!0;const f=s.fileExtension,u=s.link?UI.second_domain_for_dl?UI.downloaddomain+s.link:window.location.origin+s.link:null,w=encodeURIComponent(s.name);let m=i+w+"?a=view";const h=!window.UI||window.UI.user_role==="admin"||window.UI.can_stream||window.UI.can_download,y=!window.UI||window.UI.user_role==="admin"||window.UI.can_download,_=i+w;n&&s.name=="README.md"&&UI.render_readme_md&&get_file(_,s,function(v){markdown("#readme_md",v),$("img").addClass("img-fluid")}),s.name=="HEAD.md"&&UI.render_head_md&&get_file(_,s,function(v){markdown("#head_md",v),$("img").addClass("img-fluid")});const b=GDOC_TYPES[s.mimeType],x=b?b.icon:getFileIcon(f),k=b&&UI.display_download?b.formats.map(v=>`<a class="gdi-act-btn" href="${u}&fmt=${v.ext}" title="Export as ${v.label}" download><span style="font-size:10px;font-weight:600;">${v.label}</span></a>`).join(""):"";c+=`<div class="gdi-row countitems size_items" data-name="${escHtml(s.name.toLowerCase())}" data-bytes="${g}" data-date="${s.modifiedTime||""}">
  ${h&&UI.allow_selecting_files?`<input class="gdi-row-check" type="checkbox" value="${u}">`:""}
  <span class="gdi-row-icon">${x}</span>
  ${h?`<a class="gdi-row-name" href="${m}" title="${escHtml(s.name)}" data-size="${UI.display_size?s.size:""}">${escHtml(s.name)}</a>`:`<span class="gdi-row-name gdi-row-name--locked" title="${escHtml(s.name)}" data-size="${UI.display_size?s.size:""}">${escHtml(s.name)}</span>`}
  <span class="gdi-row-size">${UI.display_size?s.size:""}</span>
  <span class="gdi-row-date">${UI.display_time?s.modifiedTime:""}</span>
  <span class="gdi-row-acts">
    ${h&&UI.allow_selecting_files?`<button class="gdi-act-btn" onclick="copyShareUrl(this.closest('.gdi-row').querySelector('.gdi-row-name').href)" title="Copy link"><i class="bi bi-link-45deg"></i></button>`:""}
  ...
  <div class="gdi-viewer">
    ...
    <div class="gdi-code-outer" style="display:none;">
      <pre><code id="editor"></code></pre>
              ...
              ${o.map(w=>`<track kind="subtitles" src="${escHtml(w.url)}" label="${escHtml(w.label)}" default>`).join("")}
